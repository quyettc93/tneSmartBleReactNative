...
export default function App() {
  ...
  const [callBinary, setCallBinary] = useState([]);
  const [funcBinary, setFuncBinary] = useState([]);
  ...
  console.log("App -> blemanager", button, isConnected);

  // useEffect(() => {
  //   if (callBinary.length !== 0 && isConnected) {
  //     writeDataToDevice(serviceUUID, characteristicUUID, value);
  //     setCallBinary([]);
  //   }
  // }, [callBinary, isConnected]);

  useEffect(() => {
    if (idFromQr !== null && deviceConnect !== null) {
  ...
  const scanForDevices = async () => {
    try {
      ...
      setDeviceConnect([]); // Xóa danh sách thiết bị cũ
      ...
      blemanager.startDeviceScan(null, null, async (error, device) => {
        ...
        if (
          ...
          console.log("Tìm thấy thiết bị:", device.localName, device);
          blemanager.stopDeviceScan(); // Dừng quét ngay khi tìm thấy thiết bị
          await connectToPeripheral(device); // Kết nối với thiết bị
        }
      });
    } catch (error) {
  ...
  const connectToPeripheral = async (device) => {
    try {
      ...
      // Khám phá các dịch vụ và đặc tính
      await connectedDevice.discoverAllServicesAndCharacteristics();
      // Lưu thiết bị đã kết nối vào state
      setDeviceConnect(connectedDevice);
      setIsConnected(true);
  ...
  //     await deviceConnect.writeCharacteristicWithResponseForService(
  //       SERVICE_UUID,
  //       CHARACTERISTIC_UUID,
  //       encodedMessage
  //     );
  ...
  const handleButtonPress = (buttonNumber) => {
    console.log("Press button", buttonNumber);
