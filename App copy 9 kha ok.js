...
export default function App() {
  ...
  const scanForPeripherals = async () => {
    ...
    bleManager.startDeviceScan(null, null, (error, device) => {
      ...
      if (
        ...
        setConnectedDevice(device); // Lưu lại thiết bị đã kết nối
        setIsConnected(true);
  ...
  const connectToPeripheral = async (device) => {
    ...
    try {
      ...
      // Cập nhật trạng thái sau khi kết nối và khám phá dịch vụ thành công
    } catch (error) {
  ...
  // Hàm gửi dữ liệu
  const writeDataToDevice = async (value) => {
    ...
    try {
      console.log("Thử Gửi dữ liệu:", value);
